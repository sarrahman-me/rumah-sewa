...
// DetailDrawer shows per-house payment breakdowns; formatting only, no behavior changes.

import * as React from 'react';

import { idr } from '@/lib/format';
import { isoToMonth, monthToISOFirst } from '@/lib/period';
...
export function DetailDrawer({
  ...
  const dueRecord =
    ...
    ((tab === 'rent' ? { rent_due: 0 } : { water_due: 0 }) as Partial<RentStatus & WaterStatus>);
  const dueValue =
    tab === 'rent'
  ...
  async function submitPayment() {
    ...
    if (dueValue > 0 && amount > dueValue + 0.0001) {
      const proceed = confirm('Nominal melebihi tunggakan. Lanjutkan tetap menyimpan?');
      if (!proceed) return;
    }
    const payload: any = {
      house_id: house!.id,
  ...
  async function markLunas() {
    ...
    const proceed = confirm(
      `Tandai ${tab === 'rent' ? 'sewa' : 'air'} lunas sebesar ${idr(dueValue)}?`
    );
    if (!proceed) return;
    const payload: any = {
      house_id: house!.id,
    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/40 backdrop-blur-sm">
      <div className="relative flex h-full w-full max-w-md flex-col overflow-y-auto bg-white shadow-xl sm:max-w-lg">
        <div className="flex flex-col gap-3 border-b border-blue-100 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            <p className="text-xs uppercase tracking-wide text-blue-500">Detail Rumah</p>
        ...
        <div className="space-y-6 px-4 py-6 sm:px-6">
          ...
          <section>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <h3 className="text-sm font-semibold text-blue-700">Riwayat Pembayaran</h3>
                  ...
                  {payments.map((pay) => (
                    <tr key={pay.id} className="border-t border-blue-100">
                      <td className="px-3 py-2 text-slate-600">
                        <div className="font-medium text-slate-800">{isoToMonth(pay.period)}</div>
                        <div className="text-xs text-slate-400">
                          dibuat {toDateTime(pay.created_at)}
                        </div>
                      </td>
                      <td className="px-3 py-2 font-semibold text-blue-700">{idr(pay.amount)}</td>
                      <td className="px-3 py-2 text-slate-600">{toDate(pay.paid_at)}</td>
                      <td className="px-3 py-2 text-slate-600">{pay.method ?? '-'}</td>
